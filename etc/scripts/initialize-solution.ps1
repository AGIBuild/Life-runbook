[CmdletBinding()]
param(
    [string] $OpenIddictCertificatePassPhrase = $env:OPENIDDICT_CERTIFICATE_PASSPHRASE
)
...
if ([string]::IsNullOrWhiteSpace($OpenIddictCertificatePassPhrase)) {
    [Console]::Error.WriteLine("OPENIDDICT_CERTIFICATE_PASSPHRASE is required to generate openiddict.pfx")
    exit -1
}
...
$jobs += Start-Job -Name "DbMigrator" -ScriptBlock {
    $ErrorActionPreference = "Stop"
    Set-Location (Join-Path $using:scriptRoot "../../src/Agi.LifeRunbook.DbMigrator")
    dotnet run
    dotnet run
...
$jobs += Start-Job -Name "DevCert" -ScriptBlock {
    $ErrorActionPreference = "Stop"
    Set-Location (Join-Path $using:scriptRoot "../../src/Agi.LifeRunbook.Web")
    dotnet dev-certs https -v -ep openiddict.pfx -p $using:OpenIddictCertificatePassPhrase
